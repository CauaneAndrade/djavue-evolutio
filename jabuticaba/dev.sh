...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=jabuticaba.example.com
DK_COMPOSE=docker/compose/dev.yaml
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development jabuticaba npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    ...
    docker run --name jabuticaba -d --env-file /home/ubuntu/jabuticaba.env \
...
function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./jabuticaba/
...
echo_green "Welcome to jabuticaba's dev env"
